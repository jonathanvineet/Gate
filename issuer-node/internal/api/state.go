...
func (s *Server) GetStateTransactions(ctx context.Context, request GetStateTransactionsRequestObject) (GetStateTransactionsResponseObject, error) {
	...
	if err != nil {
		...
		// Return proper HTTP error status with JSON error message and non-nil error to trigger 500 status
		return GetStateTransactions500JSONResponse{N500JSONResponse{Message: err.Error()}}, err
	}
